...
const [selectedConsultantName, setSelectedConsultantName] = useState("");
    ...
    const generateNewWorkOrderId = useCallback(async () => {
      try {
        ...
        if (employeeError) {
          console.error("Error fetching employee emails:", employeeError.message);
          return;
        }
    
        // Example: if employeeData = [
        //   { email: "bgc@opdmail.com" },
        //   { email: "bc@opdmail.com" },
        //   { email: "nc@opdmail.com" },
        //   { email: "01@opdmail.com" },
        //   { email: "03@opdmail.com" }
        // ]
        // then employeeIds = ["bgc", "bc", "nc", "01", "03"]
        const employeeIds = employeeData
          .filter((emp) => emp.email.endsWith("@opdmail.com"))
        ...
        console.log("Employee IDs for branch:", employeeIds);
    
        // Filter out only numeric IDs
        const numericEmployeeIds = employeeIds.filter((id) => /^[0-9]+$/.test(id));
        if (numericEmployeeIds.length === 0) {
          console.warn("No numeric employee IDs found for this branch.");
          return;
        }
    
        // Sort them numerically and pick the largest
        numericEmployeeIds.sort((a, b) => parseInt(a, 10) - parseInt(b, 10));
        const employeeId = numericEmployeeIds[numericEmployeeIds.length - 1];
        console.log("Using numeric employee ID for Work Order:", employeeId);
    
        // Define default starting Work Order IDs per branch
        const branchDefaultIds = {
          TVR: 1, // Trivandrum
          NTA: 1, // Neyyantinkara
          KOT1: 1, // Kottarakara 1
          KOT2: 1, // Kottarakara 2
          KAT: 1, // Kattakada
        ...
        let newWorkOrderIdIncrement = branchDefaultIds[branch] || 1;
    
        if (lastWorkOrders && lastWorkOrders.length > 0) {
          const lastWorkOrderId = lastWorkOrders[0].work_order_id;
    
          const match = lastWorkOrderId.match(/CRW-\d+-(\d+)/);
          if (match && match[1]) {
            const lastIncrement = parseInt(match[1], 10);
            if (!isNaN(lastIncrement)) {
              newWorkOrderIdIncrement = lastIncrement + 1;
            } else {
              console.warn(
                "Invalid lastWorkOrderId increment, defaulting to:",
                newWorkOrderIdIncrement
              );
            }
          } else {
            console.warn(
              "Last Work Order ID format invalid, defaulting to:",
              newWorkOrderIdIncrement
            );
          }
        }
    
        // Construct the new Work Order ID with the largest numeric employee ID
        const newWorkOrderId = `CRW-${employeeId}-${newWorkOrderIdIncrement
          .toString()
          .padStart(3, "0")}`;
        ...
        dispatch({
          ...
          payload: {
            ...
            employeeIds, // Include all employee IDs in the payload if needed
          },
        });
    
        console.log("Generated Work Order ID:", newWorkOrderId);
        console.log("Dispatched Employee IDs:", employeeIds);
      } catch (error) {
    ...
    const validateField = useCallback(
      (index, field) => {
        ...
        if (field === "id" && !productEntries[index].id) {
          errors[`productId-${index}`] = "Service ID is required";
    ...
    const nextStep = useCallback(() => {
      ...
      if (step === 1) {
        ...
        if (hasMrNumber === null) {
          alert("Error: cannot proceed, please contact reception");
    ...
    const saveWorkOrder = useCallback(async () => {
      ...
      productEntries.forEach((product, index) => {
        if (!product.id)
          productErrors[`productId-${index}`] = "Service ID is required.";
        if (!product.price)
      ...
      try {
        ...
        let payload = {
          ...
          is_b2b: isB2B,
          insurance_name: isB2B ? gstNumber : null,
        ...
        if (isEditing) {
          ...
          if (error) {
            ...
            try {
              ...
              if (modReqError) {
                console.error(
                  "Error fetching modification request:",
                  modReqError.message
                );
                alert(
                ...
                if (modificationError) {
                  ...
                  console.log(
                    "Modification request status updated to 'completed'."
                  );
                  alert("Modification request completed successfully.");
                }
              } else {
                console.log(
                  "No pending or approved modification request found for this work order."
                );
                // Optionally, handle cases where no modification_request exists
              }
            } catch (err) {
              console.error(
                "Unexpected error updating modification request:",
                err
              );
              alert(
          ...
          const { error } = await supabase.from("counselling").insert(payload);
                  ...
                  <div key={index} className="flex space-x-2 items-center">
                    ...
                    <div className="relative w-2/4">
                      <input
                        type="text"
                        id={`productId-${index}`}
                        placeholder="Enter Service ID or Scan Barcode"
                        value={product.id}
                        onChange={async (e) => {
                        ...
                        aria-label={`Service ID input ${index + 1}`}
                      />
              ...
              <div className="flex items-center space-x-4">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    dispatch({
                      type: "SET_WORK_ORDER_FORM",
                      payload: { hasMrNumber: null }, // Set to null to avoid rendering fields
                    });
                  }}
...
{isB2B && (
  <div className="relative w-full mt-4">
    ...
    <input
      ...
      aria-label="Enter Insurance Name"
    />
  ...
  <select
    ...
    onChange={(e) => setSelectedConsultantName(e.target.value)}
    className="border border-gray-300 px-4 py-2 rounded-lg ml-2"
  >
    <option value="">Select Consultant</option>
    {consultantOptions.map((consultant) => (
      <option key={consultant.s_idCR} value={consultant.s_name}>
        {consultant.s_name}
      </option>
    ))}
  </select>
