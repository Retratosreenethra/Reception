...
// Main Component
const CaptureCRGeneration = memo(({ isCollapsed, onModificationSuccess }) => {
  ...
  const generateSalesOrderId = async () => {
    try {
      ...
      // If data exists, extract and increment the last number
      if (data && data.length > 0) {
        const lastSalesOrderId = data[0].sales_order_id;
        const match = lastSalesOrderId.match(/OPS-01-(\d{3})/);
      ...
      // Format the new Sales Order ID (padded to 3 digits)
      const newSalesOrderId = `CRS-01-${newOrderNumber
        .toString()
        .padStart(3, "0")}`;
  ...
  const focusFirstFieldOfStep = () => {
    ...
    if (step === 3) employeeRef.current?.focus();
    if (step === 4) {
    ...
    try {
      ...
      if (isEditing) {
        ...
        if (fetchUpdatedError || !updatedSalesOrderData) {
          console.error(
            "Error fetching updated sales order ID:",
            fetchUpdatedError
          );
          ...
          try {
            ...
            if (modReqError) {
              ...
              console.log(
                "No pending modification request found for this sales order."
              );
            }
          } catch (err) {
            console.error(
                    ...
                    >
                      ...
                      <div className="relative w-2/4">
                        <input
                          ...
                          min="1" // Allow minimum quantity of 1
                          max={product.stock} // Prevent ordering more than available stock
                          ref={(el) => (quantityRefs.current[index] = el)}
                          onKeyDown={(e) => {
                            if (e.key === "Enter") {
                              e.preventDefault();
                              nextStep(); // Move to the next step immediately
                            }
                          }}
                        />
                      </div>
          ...
          <div className="flex justify-center mt-6">
            ...
            {step < 5 && (
              <button
                ...
                onClick={nextStep}
                className={`bg-green-500 hover:bg-green-600 text-white mx-2 px-4 py-2 rounded-lg ${
                  step === 3 && !salesOrderForm.isPinVerified
                    ? "opacity-50 cursor-not-allowed"
                    : ""
                }`}
...
export default CaptureCRGeneration;
